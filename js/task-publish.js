...
(function() {
	...
	var starIndex = 0;
	var feedback = {
		component: document.getElementById('txtComponent'),
		taskName: document.getElementById('txtTaskName'),
		description: document.getElementById('txtTaskDescription'),
		handlePerson: document.getElementById('txtHandlePerson'),
		handleTime: document.getElementById('txtHandleTime'),
	...
	feedback.clearForm = function() {
		...
		feedback.taskName.value = '';
		feedback.description.value = '';
		feedback.handlePerson.value = '';
		feedback.handleTime.value = '';
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		...
		//		}
		if(feedback.component.value == '') {
			return mui.toast('无效的构建信息')
		}
		if(feedback.taskName.value == '') {
			return mui.toast('任务名称不能为空')
		}
		if(feedback.handlePerson.value == '') {
			return mui.toast('处理人不能为空')
		}
		if(feedback.handleTime.value == '') {
			return mui.toast('处理时间不能为空')
		}
		//判断网络连接
		...
		feedback.send(mui.extend({}, feedback.deviceInfo, {
			component: feedback.component.value,
			taskName: feedback.taskName.value,
			description: feedback.description.value,
			handlePerson: feedback.handlePerson.value,
			handleTime: feedback.handleTime.value,
	...
	feedback.send = function(content) {
		...
		mui.alert("新建任务成功，点击确定关闭", "新建任务", "确定", function() {
			feedback.clearForm();
			mui.back();
		});
